...
public class IAFacile extends Entity {
	...
	public IAFacile(Sprite sprite, Vector2f pos, int size) {
	...
	private void chercherDestination() {
		if(this.attaque) {
			Sommet temp = null;
			for(int i = 0; i < PlayState.iaFacile.length; i++) {
				if(PlayState.iaFacile[i] != this) {
					if(PlayState.iaFacile[i] != null) {
						int positionX = (int) PlayState.iaFacile[i].getSaCase().getPos().x;
						int positionY = (int) PlayState.iaFacile[i].getSaCase().getPos().y;
	...
	/** Fonction qui permet de supprimer l'iaFacile du PlayState */
	@Override
	protected void meurt() {
		for(int i = 0; i < PlayState.iaFacile.length; i++) {
			if(PlayState.iaFacile[i] == this) {
				PlayState.iaFacile[i] = null;
			}
		}
	}
}