    	...
    	for(int i = 0; i < matrice.length ; i++) {
    		for(int j = 0; j < matrice[i].length; j++) {
    			if(TileMap.tmo_blocks.containsKey(String.valueOf(i)+","+String.valueOf(j))) {
    				...
    				if(block instanceof ObstacleBlock) { matrice[i][j] = 1; } 
    				else if(block instanceof BreakableBlock) { matrice[i][j] = 2;}
    			}
    			
    			if(TileMap.tmo_bonus.containsKey(String.valueOf(i)+","+String.valueOf(j))) { matrice[i][j] = 7;} 
    			else if (TileMap.ground.containsKey(String.valueOf(i)+","+String.valueOf(j))) { matrice[i][j] = 3;}
