...
public class TileCollision {
	...
	/* Fonction de collision avec les flammes sur les briques cassables */
	public boolean collisionFire(float ax, float ay) {
		...
		for(int c = 0; c < 4; c++) {
			...
			if(TileMapBlock.tmo_blocks.containsKey(String.valueOf(xt) + "," + String.valueOf(yt))) {
				...
				if(block instanceof BreakableBlock) {
					System.out.println(TileMapBlock.tmo_blocks.get(String.valueOf(xt) + "," + String.valueOf(yt)).update(e.getBounds()));
					return TileMapBlock.tmo_blocks.get(String.valueOf(xt) + "," + String.valueOf(yt)).update(e.getBounds());
				}	
			} 
		}
